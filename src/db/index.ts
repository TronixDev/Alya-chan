...
import type { ChatbotSetup } from "#alya/types";
import * as schema from "./schema";

if (Environment.DatabaseLocalPath) {
	const dir = dirname(Environment.DatabaseLocalPath);
	if (!existsSync(dir)) {
...
const client = createClient({
	...
	syncUrl: Environment.DatabaseLocalPath,
	authToken: Environment.DatabasePassword,
	syncInterval: Environment.DatabaseSyncInterval,
});
...
export class AlyaDatabase {
	 ...
	 * Only applicable if embedded replicas are enabled.
	 */
	public async sync() {
		// client.sync is only available on the client instance
	 ...
	 */
	public async getChatbotSetup(guildId: string): Promise<ChatbotSetup | null> {
		const data = await this.db
			.select()
			.from(schema.guild)
			.where(eq(schema.guild.id, guildId))
	...
	public async getUserSetting(
		...
		const row = await this.db
			...
			.where(
				and(
					eq(schema.userSettings.guildId, guildId),
					eq(schema.userSettings.userId, userId),
					eq(schema.userSettings.namespace, namespace),
	...
	public async deleteUserSetting(
		guildId: string,
		userId: string,
		namespace: string,
		key: string,
	...
	public async getUserChatbotLocale(
		guildId: string,
		userId: string,
	): Promise<string | null> {
		const row = await this.db
			.select()
			.from(schema.userChatbotLocale)
			.where(
				and(
					eq(schema.userChatbotLocale.guildId, guildId),
					eq(schema.userChatbotLocale.userId, userId),
				),
			)
			.get();
	...
	public async setUserChatbotLocale(
		guildId: string,
		userId: string,
		locale: string,
	): Promise<void> {
		await this.db
			.insert(schema.userChatbotLocale)
			.values({ id: randomUUID(), guildId, userId, locale })
			.onConflictDoUpdate({
				target: schema.userChatbotLocale.id,
				set: { locale },
			});
	}

	public async deleteUserChatbotLocale(
		guildId: string,
		userId: string,
	): Promise<void> {
		await this.db
			.delete(schema.userChatbotLocale)
			.where(
				and(
					eq(schema.userChatbotLocale.guildId, guildId),
					eq(schema.userChatbotLocale.userId, userId),
	...
	public async getUserChatbotLanguage(user_id: string): Promise<string | null> {
		const row = await this.db
			.select()
			.from(schema.userPreferences)
			.where(eq(schema.userPreferences.userId, user_id))
			.get();
		return row?.chatbotLanguage ?? null;
	}

	public async setUserChatbotLanguage(
		user_id: string,
		language: string,
	): Promise<void> {
		await this.db
