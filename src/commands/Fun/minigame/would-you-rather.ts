		...
		const fetchQuestion = async (): Promise<boolean> => {
			...
			try {
				...
				const data = (await response.json()) as {
					statements?: Array<{
						title: string;
						phrase: Array<{ text: string; count?: number }>;
					}>;
				};
				if (data.statements && data.statements.length > 0) {
					const random =
						data.statements[Math.floor(Math.random() * data.statements.length)];
					const phrase0 = random?.phrase?.[0];
					const phrase1 = random?.phrase?.[1];
					if (!random?.title || !phrase0 || !phrase1) {
						// fall through to next API
					} else {
						questionData = {
							question: random.title,
							option1: phrase0.text,
							option2: phrase1.text,
							votes: {
								option1: phrase0.count || 0,
								option2: phrase1.count || 0,
							},
			...
			try {
				const response = await ky.get("https://api.truthordarebot.xyz/v1/wyr", {
					throwHttpErrors: false,
				});
				...
				if (data.question) {
					...
					if (
						...
						questionData = {
							...
							option1: part0.replace("Would you rather", "").trim(),
							option2: part1.replace("?", "").trim(),
