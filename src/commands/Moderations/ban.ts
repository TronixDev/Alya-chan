...
const option = {
	user: createUserOption({
		...
		required: true,
		locales: {
			name: "cmd.ban.options.user.name",
			description: "cmd.ban.options.user.description",
		},
	}),
	reason: createStringOption({
		...
		required: false,
		locales: {
			name: "cmd.ban.options.reason.name",
			description: "cmd.ban.options.reason.description",
		},
	}),
};
...
@LocalesT("cmd.ban.name", "cmd.ban.description")
@Options(option)
	...
	async run(ctx: CommandContext<typeof option>): Promise<void> {
			...
			const components = new Container()
				...
				.addComponents(
					new TextDisplay().setContent(
						`${client.config.emoji.no} ${cmd.ban.run.cant_self}`,
					),
				);
		...
		try {
			await guild.members.ban(
				target.id,
				reason ? { delete_message_seconds: 0 } : undefined,
				reason,
			);

			const successMessage = cmd.ban.run.success({
				user: target.tag,
				reason,
			});
			const components = new Container()
			...
			const failedMessage = cmd.ban.run.failed({ error: message });
