	...
	async run(ctx: CommandContext<typeof option>) {
		...
		const getCategoryEmoji = (category: number): string => {
			switch (category) {
				...
				case AlyaCategory.Fun:
					return client.config.emoji.party;
				case AlyaCategory.Utils:
					return client.config.emoji.globe;
				case AlyaCategory.Moderations:
					return client.config.emoji.settings;
				default:
